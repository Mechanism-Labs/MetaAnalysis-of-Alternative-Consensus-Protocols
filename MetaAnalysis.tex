...
Ouroboros Genesis selects proposers by employing a random generation scheme that uses a VRF. Each user in the network locally evaluates the VRF using their secret key and the inputs (current slot index and epoch randomness). If the output of the VRF is less than some threshold T, then the user is eligible to be leader. The user then attaches the  output and proof (both generated from the VRF) to the new block that is being proposed before being propagated to the network.
\\\\
The randomness seed is updated at the beginning of each epoch; the randomness seed of the epoch is derived from hashing the previous epoch's randomness, the current epoch number, and the output of certain values from the first 2/3 blocks of the previous epoch. It follows this equation:
$ \eta_{ep} \leftarrow H(\eta_{ep-1} || ep || v) $
\\\\ 
...
\subsubsection{Ouroboros Genesis}
The slot proposer is only revealed at the time of block propagation when the proposer attaches their VRF output and proof with the block. This ensures Ouroboros Genesis can defend against DoS attacks on the proposers and is adaptively secure in a model with erasures, even with instantaneous corruption.
\subsubsection{Casper FFG}
...
\bibitem{BLS} D. Boneh, Ben Lynn and Hovav Shacham (2004). "Short Signatures from the Weil Pairing". Journal of Cryptology. 17: 297–319. doi:10.1007/s00145-004-0314-9.
