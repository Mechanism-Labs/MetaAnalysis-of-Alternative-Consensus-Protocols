...
\alberto{If you wish I can add sections about S-BAC and Avalanche.}
\alberto{What about splitting this big latex file in one file per chapter (to make merge conflicts easier to solve)?}
\alberto{What is the page limit?}
...
\alberto{Introduce PoS gently.}
In a \emph{Proof of Stake} (PoS) system, consensus is reached through the agreement of validators who have bonded their coins to the network. These validators are akin to miners in PoW. However, in PoS, validators create a deposit (a bond) of coins which allows them to propose and vote on new blocks. 
...
\alberto{Need to refine scope (ie, we focus on Tendermint, Algorand, etc)}
\alberto{Write outline.}
\hfill 
...
Indeed, there have been academic works encouraging the shift from PoW to PoS and calling for more robust analysis frameworks, including Vukolic \cite{Vukolic}, which encourages use of classical BFT protocols over PoW for reasons of safety and fork-resistance. \note{Add more works encouraging.} Bano et al. \cite{Bano} investigate blockchain  protocols for consensus, providing a survey of different types of alternative consensus protocols that aim to be used in permissionless environments.
\alberto{Add Arthur Gervais literature.}
...
\alberto{Move citation up as the term is already introduced.}
...
\emph{Semi-synchronous} Networks also assume the existence of an upper bound not known a priori, however the distribution of the upper bound is known \cite{Attiya&Lynch}. \alberto{Re-order paragraphs.}
...
\alberto{Add citations for all the following systems (in paragraphs and table)}
\alberto{I would differentiate in the table N/A as 'not provided' and '-' as not applicable; add leggend to the table.}
\alberto{Justify in each paragraph the 'safety' and 'liveness' value in the table.}
...
\cite{SnowWhite} \alberto{Hash functions are more than that -- just cite or don't explain.} or  \emph{pseudorandom function (PRF)}, a function family which cannot be significantly distinguished by an efficient algorithm from a truly random oracle, like in Sleepy. \cite{Sleepy}.
\item Implement a Random Oracle and VRF \alberto{Expand VRF.} which is secure \indent against adaptive corruptions using Algorand's model.
...
\end{enumerate}
If either of the above does not hold, the protocol enters slow mode, falling back to the underlying blockchain and their protocol for creating blocks and propagating them. \alberto{Not clear why Byz liveness is 1/4.}
...
$t \in [f+1, n-f]$ where $f$ is the amount of signatures the adversary controls and $n$ is the total signatures in the scheme. \alberto{This is tricky, I will have a deeper though about that.}
\subsubsection{Ouroboros Genesis}
...
\alberto{Read carefully from here.}
